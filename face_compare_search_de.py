"""
Diese Flask-Anwendung in einer einzigen Datei stellt browserbasierten Gesichtsvergleich und die Suche in einer Gesichtsbibliothek in einem Programm bereit.
Sie vergleicht zwei hochgeladene Gesichtsbilder, verwaltet eine lokale Referenzbibliothek und gleicht ein Suchgesicht mit gespeicherten 128-dimensionalen Encodings ab.
Suchdatensätze werden in einem Hintergrund-Thread nacheinander in aufsteigender Datenbank-ID-Reihenfolge verarbeitet, während AJAX-Abfragen den aktuellen Fortschritt und die sortierten Ergebnisse anzeigen.
Die Gesichtserkennung verwendet über face_recognition das CNN-Modell von dlib, und für das Gesichts-Encoding wird das large-Modell mit 68 Gesichtspunkten verwendet. Jedes hochgeladene Bild muss genau ein erkennbares Gesicht enthalten. Bilddateien werden ohne Größenänderung, Zuschnitt, Formatkonvertierung, Neucodierung oder Komprimierung gespeichert; EXIF-Ausrichtungskorrektur und RGB-Konvertierung erfolgen während der Erkennung nur im Arbeitsspeicher. SQLite speichert Personennamen, Dateinamen der Referenzbilder, JSON-codierte Gesichtsvektoren und Erstellungszeitpunkte. Eine HTTP-Digest-Administratorauthentifizierung ist nur zum Registrieren oder Löschen von Einträgen in der Gesichtsbibliothek erforderlich.
"""
...
application = Flask(__name__)  # Flask-Anwendung erstellen
application.config["MAX_CONTENT_LENGTH"] = 64 * 1024 * 1024  # Anfragen auf 64 MB begrenzen, ohne den Bildinhalt zu verändern
application.config["SECRET_KEY"] = "face-compare-search-digest-secret-2026"  # Geheimnis zum Signieren der Digest-Authentifizierungsdaten

digest_authentication = HTTPDigestAuth()  # HTTP-Digest-Authentifizierung erstellen
ADMINISTRATOR_USERS = {"admin": "FaceAdmin@2026", "operator": "FaceOperator@2026"}  # Integrierte Administratorkonten; bei Bedarf bearbeiten

PROJECT_DIRECTORY = Path(__file__).resolve().parent  # Verzeichnis, das dieses Skript enthält
...
FACE_DETECTION_MODEL = "cnn"  # CNN-Gesichtsdetektor verwenden
FACE_ENCODING_MODEL = "large"  # large-Modell für Gesichts-Encodings verwenden
...
<body>
<header class="topbar"><div class="topbar-inner"><a class="brand" href="/compare">Online-Gesichtserkennung</a><nav class="navigation">{{ navigation_html|safe }}</nav></div></header>
...
<section class="hero"><h1>Gesichtsvergleich</h1><p>Laden Sie zwei Originalbilder hoch, die jeweils genau ein deutlich erkennbares Gesicht enthalten. Der Server berechnet die Ähnlichkeit mit CNN-Erkennung und dem large-Modell.</p></section>
<section class="card">
<div class="upload-panel">
<label class="upload-box"><input id="firstFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="firstPlaceholder" class="upload-placeholder"><span class="upload-icon">①</span>Erstes Originalbild hochladen</div><img id="firstPreview" class="preview-image" alt="Erstes Originalbild hochladen"></label>
<label class="upload-box"><input id="secondFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="secondPlaceholder" class="upload-placeholder"><span class="upload-icon">②</span>Zweites Originalbild hochladen</div><img id="secondPreview" class="preview-image" alt="Zweites Originalbild hochladen"></label>
</div>
<div class="action-row"><button id="compareButton" class="primary-button" type="button">Gesichter vergleichen</button><span id="compareStatus" class="status-text"></span></div>
<div id="compareResult" class="result-panel"><div class="similarity-label">Gesichtsähnlichkeit</div><div id="compareSimilarity" class="similarity-value">0.00%</div></div>
<p class="notice">Hochgeladene Dateien werden Byte für Byte ohne Größenänderung, Neucodierung oder Komprimierung gespeichert. Die Ausrichtungskorrektur und RGB-Konvertierung erfolgen nur im Arbeitsspeicher. Die angezeigte Ähnlichkeit ist keine Wahrscheinlichkeit für eine Identitätsprüfung.</p>
</section>
"""

SEARCH_PAGE_CONTENT = """
<section class="hero"><h1>Gesichtssuche</h1><p>Registrieren Sie Referenzgesichter und laden Sie anschließend ein Suchbild hoch. Der Server vergleicht die Datenbankeinträge nacheinander, während AJAX Fortschritt und Ergebnisse anzeigt.</p></section>
<section class="card">
<h2>Verwaltung der Gesichtsbibliothek</h2><p class="card-description">Jeder Eintrag speichert einen Personennamen, das ursprüngliche Referenzbild und ein vorberechnetes 128-dimensionales Encoding. Für eine Person können mehrere Referenzbilder gespeichert werden.</p>
<div class="authentication-panel"><button id="authenticateButton" class="secondary-button" type="button">Administratoranmeldung</button></div>
<div class="form-row"><input id="personName" class="text-input" type="text" maxlength="80" placeholder="Personennamen eingeben"><input id="libraryFaceImage" class="compact-upload" type="file" accept="image/jpeg,image/png,image/webp"></div>
<div class="action-row"><button id="addFaceButton" class="primary-button" type="button">Gesicht registrieren</button><span id="libraryStatus" class="status-text"></span></div>
<div class="section-divider"></div><div class="library-header"><h2>Registrierte Gesichter</h2><span id="libraryCount" class="library-count">0 Referenzbilder</span></div>
<div id="libraryList" class="library-list"><div class="empty-state">Gesichtsbibliothek wird geladen…</div></div>
</section>
<section class="card">
<h2>Sequenzielle Gesichtssuche</h2><p class="card-description">Die Suche vergleicht gespeicherte Gesichts-Encodings der Reihe nach und erkennt Gesichter in Bibliotheksbildern nicht erneut.</p>
<div class="search-layout"><div><label id="searchPlaceholder" class="search-placeholder">Klicken, um ein ursprüngliches Suchbild auszuwählen<br>Das Bild muss genau ein deutlich erkennbares Gesicht enthalten<input id="searchFaceImage" type="file" accept="image/jpeg,image/png,image/webp"></label><img id="searchPreview" class="search-preview" alt="Vorschau des Suchbilds"><div class="action-row"><button id="startSearchButton" class="primary-button" type="button">Sequenzielle Suche starten</button></div></div>
<div><div id="searchMessage" class="status-text">Warten auf Start</div><div id="progressArea" class="progress-area"><div class="progress-track"><div id="progressBar" class="progress-bar"></div></div><div class="progress-meta"><span id="progressText">0 / 0</span><span id="progressPercentage">0%</span></div><div id="currentPerson" class="current-person"></div></div><div id="searchResults" class="result-list"></div></div></div>
<p class="notice">Die Ergebnisse werden absteigend nach der vom CNN-Modell berechneten Ähnlichkeit des Gesichtsvergleichs sortiert.</p>
</section>
"""

ADMIN_AUTHENTICATION_PAGE = """
<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Administratorauthentifizierung erfolgreich</title>
...
<body><div class="card"><h1>Administratorauthentifizierung erfolgreich</h1><p>Aktueller Administrator: {{ administrator_username }}. Sie können zur Suchseite zurückkehren, um Gesichter zu registrieren oder zu löschen.</p><button type="button" onclick="window.close()">Fenster schließen</button></div></body></html>
"""
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}throw new Error(responseData.message||"Die Anfrage ist fehlgeschlagen");}
async function compareSelectedFaces(){const firstFaceFile=firstFaceInput.files[0],secondFaceFile=secondFaceInput.files[0],statusElement=document.getElementById("compareStatus"),resultElement=document.getElementById("compareResult");if(!firstFaceFile||!secondFaceFile){statusElement.textContent="Wählen Sie zuerst zwei Bilder aus";statusElement.className="status-text error-text";return;}compareButton.disabled=true;resultElement.style.display="none";statusElement.textContent="CNN-Erkennung und Gesichts-Encoding mit dem large-Modell werden ausgeführt…";statusElement.className="status-text";const requestData=new FormData();requestData.append("first_face_image",firstFaceFile);requestData.append("second_face_image",secondFaceFile);try{const response=await fetch("/api/compare",{method:"POST",body:requestData});const responseData=await readJsonResponse(response);document.getElementById("compareSimilarity").textContent=Number(responseData.similarity).toFixed(2)+"%";resultElement.style.display="block";statusElement.textContent="Vergleich abgeschlossen";statusElement.className="status-text success-text";}catch(error){statusElement.textContent=error.message;statusElement.className="status-text error-text";}finally{compareButton.disabled=false;}}
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}if(response.status===401){throw new Error("Die Administratorauthentifizierung fehlt oder ist ungültig. Verwenden Sie zuerst die Administratoranmeldung.");}throw new Error(responseData.message||"Die Anfrage ist fehlgeschlagen");}
...
function renderFaceLibrary(faceRecords){const libraryList=document.getElementById("libraryList");document.getElementById("libraryCount").textContent=faceRecords.length+" Referenzbilder";libraryList.replaceChildren();if(faceRecords.length===0){displayLibraryMessage("Die Gesichtsbibliothek ist leer.",false);return;}for(const faceRecord of faceRecords){libraryList.appendChild(createLibraryItem(faceRecord));}}
...
function openAdministratorAuthentication(){const authenticationWindow=window.open("/admin-auth","faceLibraryAuthentication","width=520,height=420");if(!authenticationWindow){setLibraryStatus("Der Browser hat das Authentifizierungsfenster blockiert. Erlauben Sie Pop-ups und versuchen Sie es erneut.",true);return;}setLibraryStatus("Geben Sie den Administrator-Benutzernamen und das Passwort im Authentifizierungsfenster ein",false);}
async function addFaceRecord(){const personName=personNameInput.value.trim(),faceImage=libraryFaceInput.files[0];if(!personName||!faceImage){setLibraryStatus("Geben Sie einen Personennamen ein und wählen Sie ein Bild aus",true);return;}addFaceButton.disabled=true;setLibraryStatus("CNN-Erkennung wird ausgeführt und das Originalbild sowie das Encoding werden gespeichert…",false);const requestData=new FormData();requestData.append("person_name",personName);requestData.append("face_image",faceImage);try{const response=await fetch("/api/library",{method:"POST",body:requestData,credentials:"same-origin"});await readJsonResponse(response);personNameInput.value="";libraryFaceInput.value="";setLibraryStatus("Registrierung abgeschlossen",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}finally{addFaceButton.disabled=false;}}
async function deleteFaceRecord(faceRecordId,personName){if(!window.confirm("Dieses Referenzbild für „"+personName+"“ löschen?")){return;}try{const response=await fetch("/api/library/"+faceRecordId,{method:"DELETE",credentials:"same-origin"});await readJsonResponse(response);setLibraryStatus("Löschen abgeschlossen",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}}
...
def get_administrator_password(username):
    return ADMINISTRATOR_USERS.get(username)  # Das für die Digest-Authentifizierung verwendete Passwort zurückgeben

@digest_authentication.error_handler
def handle_authentication_error(status_code=401):
    if request.path.startswith("/api/"):
        return jsonify({"message": "Die Digest-Authentifizierung des Administrators ist erforderlich"}), status_code
    return Response("Die Digest-Authentifizierung des Administrators ist erforderlich", status=status_code, content_type="text/plain; charset=utf-8")

# =============================================================================
# Verzeichnisse und Datenbank
# =============================================================================
...
def get_database_connection():
    database_connection = sqlite3.connect(DATABASE_PATH, timeout=30)  # Für jede Anfrage oder jeden Thread eine unabhängige SQLite-Verbindung verwenden
...
def save_uploaded_image_unchanged(uploaded_image, destination_directory, filename_prefix):
    ...
    uploaded_image.save(generated_path)  # Hochgeladene Bytes unverändert speichern; keine Größenänderung, Neucodierung oder Komprimierung
    try:
        with Image.open(generated_path) as source_image:
            source_image.verify()  # Lesbarkeit prüfen, ohne die Originaldatei zu verändern
    except (UnidentifiedImageError, OSError, ValueError) as image_error:
        generated_path.unlink(missing_ok=True)
        raise ValueError("Das Bild kann nicht gelesen werden oder die Datei ist beschädigt") from image_error
    return generated_path, generated_filename

def load_image_array_for_recognition(image_path):
    try:
        with Image.open(image_path) as source_image:
            oriented_image = ImageOps.exif_transpose(source_image)  # EXIF-Ausrichtung nur im Arbeitsspeicher korrigieren
            rgb_image = oriented_image.convert("RGB")  # Nur im Arbeitsspeicher in RGB konvertieren; das Original auf dem Datenträger bleibt unverändert
...
def extract_single_face_encoding(image_path):
    ...
    face_locations = face_recognition.face_locations(image_array, number_of_times_to_upsample=FACE_LOCATION_UPSAMPLE_COUNT, model=FACE_DETECTION_MODEL)  # Mit CNN genau ein Gesicht erkennen
    if len(face_locations) == 0:
        raise ValueError("Im Bild wurde kein deutlich erkennbares Gesicht gefunden")
    if len(face_locations) > 1:
        raise ValueError("Es wurden mehrere Gesichter erkannt; verwenden Sie ein Bild mit nur einem Gesicht")
    face_encodings = face_recognition.face_encodings(image_array, known_face_locations=face_locations, num_jitters=FACE_ENCODING_JITTER_COUNT, model=FACE_ENCODING_MODEL)  # Das 128-dimensionale Encoding mit dem large-Modell extrahieren
    if not face_encodings:
        raise ValueError("Das Gesichts-Encoding ist fehlgeschlagen; verwenden Sie ein deutlicheres Bild")
...
def run_sequential_face_search(search_job_id, search_face_encoding):
    ...
    try:
        ...
        for face_record in face_records:  # Strikt in Datenbankreihenfolge vergleichen
            person_name = str(face_record["person_name"])
        ...
        update_search_job(search_job_id, {"status": "error", "current_person": "", "message": "Gesichtssuche fehlgeschlagen: " + str(search_error)})
...
def render_application_page(page_title, active_page, page_content, page_script):
    ...
    navigation_html = '<a class="' + compare_class + '" href="/compare">Gesichtsvergleich</a><a class="' + search_class + '" href="/search">Gesichtssuche</a>'
...
def compare_page():
    return render_application_page("Gesichtsvergleich", "compare", COMPARE_PAGE_CONTENT, COMPARE_PAGE_SCRIPT)

@application.route("/search")
def search_page():
    return render_application_page("Gesichtssuche", "search", SEARCH_PAGE_CONTENT, SEARCH_PAGE_SCRIPT)
...
def compare_faces_api():
    ...
    try:
        ...
        return jsonify({"message": "Gesichtsvergleich fehlgeschlagen: " + str(unexpected_error)}), 500
    finally:
...
def add_face_library_api():
    ...
    try:
        ...
        if not person_name:
            raise ValueError("Personennamen eingeben")
        if len(person_name) > 80:
            raise ValueError("Der Personenname darf höchstens 80 Zeichen enthalten")
        ...
        remove_file_safely(stored_image_path)
        return jsonify({"message": "Gesichtsregistrierung fehlgeschlagen: " + str(unexpected_error)}), 500
...
def delete_face_library_api(face_record_id):
    ...
    if image_filename is None:
        return jsonify({"message": "Der angeforderte Gesichtseintrag ist nicht vorhanden"}), 404
...
def start_face_search_api():
    ...
    try:
        ...
        if face_library_count == 0:
            raise ValueError("Die Gesichtsbibliothek ist leer; registrieren Sie zuerst ein Gesicht")
        ...
        search_thread = Thread(target=run_sequential_face_search, args=(search_job_id, search_face_encoding.copy()), daemon=True, name="face-search-" + search_job_id[:8])  # AJAX-Abfragen während der Suche reaktionsfähig halten
        search_thread.start()
        return jsonify({"search_job_id": search_job_id}), 202
    except ValueError as validation_error:
        return jsonify({"message": str(validation_error)}), 400
    except Exception as unexpected_error:
        return jsonify({"message": "Die Suche konnte nicht gestartet werden: " + str(unexpected_error)}), 500
    finally:
...
def search_status_api(search_job_id):
    ...
    if search_job is None:
        return jsonify({"message": "Der Suchauftrag ist nicht vorhanden oder abgelaufen"}), 404
...
def request_too_large(_error):
    if request.path.startswith("/api/"):
        return jsonify({"message": "Die Upload-Anfrage ist zu groß; die maximale Anfragegröße beträgt 64 MB"}), 413
    return "Die Upload-Anfrage ist zu groß; die maximale Anfragegröße beträgt 64 MB", 413
...
if __name__ == "__main__":
    application.run(host="0.0.0.0", port=8080, debug=False, threaded=True)  # Entwicklungsserver; in der Produktion WSGI und HTTPS verwenden
