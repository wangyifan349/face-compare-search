"""
这是一个单文件 Flask 应用程序，在同一个程序中提供基于浏览器的人脸对比和人脸库搜索功能。它支持两张人脸图片的对比、本地人脸库管理，以及使用查询图片在人脸库中搜索已存储的 128 维人脸特征编码。
搜索任务由后台线程按照数据库记录 ID 的升序依次处理，AJAX 轮询实时显示搜索进度和排序结果。
人脸检测通过 face_recognition 调用 dlib 的 CNN 模型完成，人脸编码使用 large（68 点人脸关键点）模型。
每张上传的图片必须且只能包含一张可检测到的人脸。图片文件保存时不会进行缩放、裁剪、格式转换、重新编码或压缩；识别过程中仅在内存中执行 EXIF 方向校正和 RGB 色彩空间转换。
SQLite 存储人员姓名、参考图片文件名、JSON 格式的人脸特征向量以及创建时间。录入和删除人脸库记录时需要 HTTP Digest 管理员身份认证。
"""
