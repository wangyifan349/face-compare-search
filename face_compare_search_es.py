"""
Esta aplicación Flask de un solo archivo ofrece en un mismo programa comparación facial y búsqueda en una biblioteca de rostros desde el navegador.
Compara dos imágenes faciales subidas, administra una biblioteca local de referencias y contrasta un rostro de consulta con las codificaciones almacenadas de 128 dimensiones.
Los registros de búsqueda se procesan secuencialmente por orden ascendente del ID de la base de datos en un hilo en segundo plano, mientras el sondeo AJAX muestra en tiempo real el progreso y los resultados clasificados.
La detección facial utiliza el modelo CNN de dlib mediante face_recognition, y la codificación facial utiliza el modelo large de 68 puntos de referencia faciales. Cada imagen subida debe contener exactamente un rostro detectable. Los archivos de imagen se guardan sin redimensionamiento, recorte, conversión de formato, recodificación ni compresión; la corrección de orientación EXIF y la conversión a RGB se realizan únicamente en memoria durante el reconocimiento. SQLite almacena los nombres de las personas, los nombres de archivo de las imágenes de referencia, los vectores faciales codificados en JSON y las fechas de creación. La autenticación HTTP Digest de administrador solo es necesaria para registrar o eliminar elementos de la biblioteca facial.
"""
...
application = Flask(__name__)  # Crear la aplicación Flask
application.config["MAX_CONTENT_LENGTH"] = 64 * 1024 * 1024  # Limitar las solicitudes a 64 MB sin modificar el contenido de las imágenes
application.config["SECRET_KEY"] = "face-compare-search-digest-secret-2026"  # Secreto utilizado para firmar los datos de autenticación Digest

digest_authentication = HTTPDigestAuth()  # Crear el autenticador HTTP Digest
ADMINISTRATOR_USERS = {"admin": "FaceAdmin@2026", "operator": "FaceOperator@2026"}  # Cuentas de administrador integradas; edítelas según sea necesario

PROJECT_DIRECTORY = Path(__file__).resolve().parent  # Directorio que contiene este script
...
FACE_DETECTION_MODEL = "cnn"  # Usar el detector facial CNN
FACE_ENCODING_MODEL = "large"  # Usar el modelo large de codificación facial
...
<body>
<header class="topbar"><div class="topbar-inner"><a class="brand" href="/compare">Reconocimiento facial en línea</a><nav class="navigation">{{ navigation_html|safe }}</nav></div></header>
...
<section class="hero"><h1>Comparación facial</h1><p>Suba dos imágenes originales que contengan exactamente un rostro nítido cada una. El servidor utiliza detección CNN y el modelo large para calcular la similitud.</p></section>
<section class="card">
<div class="upload-panel">
<label class="upload-box"><input id="firstFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="firstPlaceholder" class="upload-placeholder"><span class="upload-icon">①</span>Subir la primera imagen original</div><img id="firstPreview" class="preview-image" alt="Subir la primera imagen original"></label>
<label class="upload-box"><input id="secondFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="secondPlaceholder" class="upload-placeholder"><span class="upload-icon">②</span>Subir la segunda imagen original</div><img id="secondPreview" class="preview-image" alt="Subir la segunda imagen original"></label>
</div>
<div class="action-row"><button id="compareButton" class="primary-button" type="button">Comparar rostros</button><span id="compareStatus" class="status-text"></span></div>
<div id="compareResult" class="result-panel"><div class="similarity-label">Similitud facial</div><div id="compareSimilarity" class="similarity-value">0.00%</div></div>
<p class="notice">Los archivos subidos se guardan byte por byte, sin redimensionamiento, recodificación ni compresión. La corrección de orientación y la conversión a RGB se realizan únicamente en memoria. La similitud mostrada no es una probabilidad de verificación de identidad.</p>
</section>
"""

SEARCH_PAGE_CONTENT = """
<section class="hero"><h1>Búsqueda facial</h1><p>Registre rostros de referencia y después suba una imagen de consulta. El servidor compara secuencialmente los registros de la base de datos mientras AJAX muestra el progreso y los resultados.</p></section>
<section class="card">
<h2>Gestión de la biblioteca facial</h2><p class="card-description">Cada registro almacena el nombre de una persona, la imagen de referencia original y una codificación precalculada de 128 dimensiones. Una persona puede tener varias imágenes de referencia.</p>
<div class="authentication-panel"><button id="authenticateButton" class="secondary-button" type="button">Inicio de sesión de administrador</button></div>
<div class="form-row"><input id="personName" class="text-input" type="text" maxlength="80" placeholder="Introduzca el nombre de la persona"><input id="libraryFaceImage" class="compact-upload" type="file" accept="image/jpeg,image/png,image/webp"></div>
<div class="action-row"><button id="addFaceButton" class="primary-button" type="button">Registrar rostro</button><span id="libraryStatus" class="status-text"></span></div>
<div class="section-divider"></div><div class="library-header"><h2>Rostros registrados</h2><span id="libraryCount" class="library-count">0 imágenes de referencia</span></div>
<div id="libraryList" class="library-list"><div class="empty-state">Cargando la biblioteca facial…</div></div>
</section>
<section class="card">
<h2>Búsqueda facial secuencial</h2><p class="card-description">La búsqueda compara en orden las codificaciones faciales guardadas y no vuelve a detectar rostros en las imágenes de la biblioteca.</p>
<div class="search-layout"><div><label id="searchPlaceholder" class="search-placeholder">Haga clic para seleccionar una imagen de consulta original<br>La imagen debe contener exactamente un rostro nítido<input id="searchFaceImage" type="file" accept="image/jpeg,image/png,image/webp"></label><img id="searchPreview" class="search-preview" alt="Vista previa de la imagen de consulta"><div class="action-row"><button id="startSearchButton" class="primary-button" type="button">Iniciar búsqueda secuencial</button></div></div>
<div><div id="searchMessage" class="status-text">En espera de inicio</div><div id="progressArea" class="progress-area"><div class="progress-track"><div id="progressBar" class="progress-bar"></div></div><div class="progress-meta"><span id="progressText">0 / 0</span><span id="progressPercentage">0%</span></div><div id="currentPerson" class="current-person"></div></div><div id="searchResults" class="result-list"></div></div></div>
<p class="notice">Los resultados se ordenan de mayor a menor según la similitud de comparación facial calculada por el modelo CNN.</p>
</section>
"""

ADMIN_AUTHENTICATION_PAGE = """
<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Autenticación de administrador correcta</title>
...
<body><div class="card"><h1>Autenticación de administrador correcta</h1><p>Administrador actual: {{ administrator_username }}. Puede volver a la página de búsqueda para registrar o eliminar rostros.</p><button type="button" onclick="window.close()">Cerrar ventana</button></div></body></html>
"""
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}throw new Error(responseData.message||"La solicitud falló");}
async function compareSelectedFaces(){const firstFaceFile=firstFaceInput.files[0],secondFaceFile=secondFaceInput.files[0],statusElement=document.getElementById("compareStatus"),resultElement=document.getElementById("compareResult");if(!firstFaceFile||!secondFaceFile){statusElement.textContent="Seleccione primero dos imágenes";statusElement.className="status-text error-text";return;}compareButton.disabled=true;resultElement.style.display="none";statusElement.textContent="Ejecutando la detección CNN y la codificación facial con el modelo large…";statusElement.className="status-text";const requestData=new FormData();requestData.append("first_face_image",firstFaceFile);requestData.append("second_face_image",secondFaceFile);try{const response=await fetch("/api/compare",{method:"POST",body:requestData});const responseData=await readJsonResponse(response);document.getElementById("compareSimilarity").textContent=Number(responseData.similarity).toFixed(2)+"%";resultElement.style.display="block";statusElement.textContent="Comparación completada";statusElement.className="status-text success-text";}catch(error){statusElement.textContent=error.message;statusElement.className="status-text error-text";}finally{compareButton.disabled=false;}}
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}if(response.status===401){throw new Error("La autenticación de administrador no está disponible o no es válida. Inicie primero la sesión de administrador.");}throw new Error(responseData.message||"La solicitud falló");}
...
function renderFaceLibrary(faceRecords){const libraryList=document.getElementById("libraryList");document.getElementById("libraryCount").textContent=faceRecords.length+" imágenes de referencia";libraryList.replaceChildren();if(faceRecords.length===0){displayLibraryMessage("La biblioteca facial está vacía.",false);return;}for(const faceRecord of faceRecords){libraryList.appendChild(createLibraryItem(faceRecord));}}
...
function openAdministratorAuthentication(){const authenticationWindow=window.open("/admin-auth","faceLibraryAuthentication","width=520,height=420");if(!authenticationWindow){setLibraryStatus("El navegador bloqueó la ventana de autenticación. Permita las ventanas emergentes e inténtelo de nuevo.",true);return;}setLibraryStatus("Introduzca el nombre de usuario y la contraseña del administrador en la ventana de autenticación",false);}
async function addFaceRecord(){const personName=personNameInput.value.trim(),faceImage=libraryFaceInput.files[0];if(!personName||!faceImage){setLibraryStatus("Introduzca el nombre de una persona y seleccione una imagen",true);return;}addFaceButton.disabled=true;setLibraryStatus("Ejecutando la detección CNN y guardando la imagen original y la codificación…",false);const requestData=new FormData();requestData.append("person_name",personName);requestData.append("face_image",faceImage);try{const response=await fetch("/api/library",{method:"POST",body:requestData,credentials:"same-origin"});await readJsonResponse(response);personNameInput.value="";libraryFaceInput.value="";setLibraryStatus("Registro completado",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}finally{addFaceButton.disabled=false;}}
async function deleteFaceRecord(faceRecordId,personName){if(!window.confirm("¿Eliminar esta imagen de referencia de «"+personName+"»?")){return;}try{const response=await fetch("/api/library/"+faceRecordId,{method:"DELETE",credentials:"same-origin"});await readJsonResponse(response);setLibraryStatus("Eliminación completada",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}}
...
def get_administrator_password(username):
    return ADMINISTRATOR_USERS.get(username)  # Devolver la contraseña utilizada por la autenticación Digest

@digest_authentication.error_handler
def handle_authentication_error(status_code=401):
    if request.path.startswith("/api/"):
        return jsonify({"message": "Se requiere autenticación Digest de administrador"}), status_code
    return Response("Se requiere autenticación Digest de administrador", status=status_code, content_type="text/plain; charset=utf-8")
...
def get_database_connection():
    database_connection = sqlite3.connect(DATABASE_PATH, timeout=30)  # Usar una conexión SQLite independiente por solicitud o hilo
...
def save_uploaded_image_unchanged(uploaded_image, destination_directory, filename_prefix):
    ...
    uploaded_image.save(generated_path)  # Guardar los bytes subidos sin cambios, redimensionamiento, recodificación ni compresión
    try:
        with Image.open(generated_path) as source_image:
            source_image.verify()  # Verificar que el archivo se pueda leer sin modificar el original
    except (UnidentifiedImageError, OSError, ValueError) as image_error:
        generated_path.unlink(missing_ok=True)
        raise ValueError("No se puede leer la imagen o el archivo está dañado") from image_error
    return generated_path, generated_filename

def load_image_array_for_recognition(image_path):
    try:
        with Image.open(image_path) as source_image:
            oriented_image = ImageOps.exif_transpose(source_image)  # Corregir la orientación EXIF únicamente en memoria
            rgb_image = oriented_image.convert("RGB")  # Convertir a RGB únicamente en memoria; el original del disco permanece sin cambios
...
def extract_single_face_encoding(image_path):
    ...
    face_locations = face_recognition.face_locations(image_array, number_of_times_to_upsample=FACE_LOCATION_UPSAMPLE_COUNT, model=FACE_DETECTION_MODEL)  # Detectar exactamente un rostro con CNN
    if len(face_locations) == 0:
        raise ValueError("No se detectó ningún rostro nítido en la imagen")
    if len(face_locations) > 1:
        raise ValueError("Se detectaron varios rostros; utilice una imagen que contenga un solo rostro")
    face_encodings = face_recognition.face_encodings(image_array, known_face_locations=face_locations, num_jitters=FACE_ENCODING_JITTER_COUNT, model=FACE_ENCODING_MODEL)  # Extraer la codificación de 128 dimensiones con el modelo large
    if not face_encodings:
        raise ValueError("La codificación facial falló; utilice una imagen más nítida")
...
# =============================================================================
# Tareas de búsqueda en segundo plano
# =============================================================================
...
def run_sequential_face_search(search_job_id, search_face_encoding):
    ...
    try:
        ...
        for face_record in face_records:  # Comparar estrictamente en el orden de la base de datos
            person_name = str(face_record["person_name"])
        ...
        update_search_job(search_job_id, {"status": "error", "current_person": "", "message": "La búsqueda facial falló: " + str(search_error)})

# =============================================================================
# Rutas de páginas y de la API
# =============================================================================
...
def render_application_page(page_title, active_page, page_content, page_script):
    ...
    navigation_html = '<a class="' + compare_class + '" href="/compare">Comparación facial</a><a class="' + search_class + '" href="/search">Búsqueda facial</a>'
...
def compare_page():
    return render_application_page("Comparación facial", "compare", COMPARE_PAGE_CONTENT, COMPARE_PAGE_SCRIPT)

@application.route("/search")
def search_page():
    return render_application_page("Búsqueda facial", "search", SEARCH_PAGE_CONTENT, SEARCH_PAGE_SCRIPT)
...
def compare_faces_api():
    ...
    try:
        ...
        return jsonify({"message": "La comparación facial falló: " + str(unexpected_error)}), 500
    finally:
...
def add_face_library_api():
    ...
    try:
        ...
        if not person_name:
            raise ValueError("Introduzca el nombre de la persona")
        if len(person_name) > 80:
            raise ValueError("El nombre de la persona no puede superar los 80 caracteres")
        ...
        remove_file_safely(stored_image_path)
        return jsonify({"message": "El registro del rostro falló: " + str(unexpected_error)}), 500
...
def delete_face_library_api(face_record_id):
    ...
    if image_filename is None:
        return jsonify({"message": "El registro facial solicitado no existe"}), 404
...
def start_face_search_api():
    ...
    try:
        ...
        if face_library_count == 0:
            raise ValueError("La biblioteca facial está vacía; registre primero un rostro")
        ...
        search_thread = Thread(target=run_sequential_face_search, args=(search_job_id, search_face_encoding.copy()), daemon=True, name="face-search-" + search_job_id[:8])  # Mantener receptivo el sondeo AJAX durante la búsqueda
        search_thread.start()
        return jsonify({"search_job_id": search_job_id}), 202
    except ValueError as validation_error:
        return jsonify({"message": str(validation_error)}), 400
    except Exception as unexpected_error:
        return jsonify({"message": "No se pudo iniciar la búsqueda: " + str(unexpected_error)}), 500
    finally:
...
def search_status_api(search_job_id):
    ...
    if search_job is None:
        return jsonify({"message": "La tarea de búsqueda no existe o ha caducado"}), 404
...
def request_too_large(_error):
    if request.path.startswith("/api/"):
        return jsonify({"message": "La solicitud enviada es demasiado grande; el tamaño máximo permitido es de 64 MB"}), 413
    return "La solicitud enviada es demasiado grande; el tamaño máximo permitido es de 64 MB", 413
...
if __name__ == "__main__":
    application.run(host="0.0.0.0", port=8080, debug=False, threaded=True)  # Servidor de desarrollo; use WSGI y HTTPS en producción
