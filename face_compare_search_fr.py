"""
Cette application Flask monofichier réunit dans un même programme une comparaison de visages et une recherche dans une bibliothèque faciale accessibles depuis un navigateur.
Elle compare deux images de visage téléversées, gère une bibliothèque locale de références et confronte un visage de recherche aux encodages à 128 dimensions enregistrés.
Les enregistrements de recherche sont traités séquentiellement par identifiant de base de données croissant dans un thread d’arrière-plan, tandis qu’une interrogation AJAX affiche en temps réel la progression et les résultats classés.
La détection des visages utilise le modèle CNN de dlib par l’intermédiaire de face_recognition, et l’encodage facial utilise le modèle large à 68 points de repère faciaux. Chaque image téléversée doit contenir exactement un visage détectable. Les fichiers image sont enregistrés sans redimensionnement, recadrage, conversion de format, réencodage ni compression ; la correction de l’orientation EXIF et la conversion RGB sont effectuées uniquement en mémoire pendant la reconnaissance. SQLite conserve les noms des personnes, les noms de fichiers des images de référence, les vecteurs faciaux encodés en JSON et les dates de création. L’authentification administrateur HTTP Digest est requise uniquement pour enregistrer ou supprimer des éléments de la bibliothèque faciale.
"""
...
application = Flask(__name__)  # Créer l’application Flask
application.config["MAX_CONTENT_LENGTH"] = 64 * 1024 * 1024  # Limiter les requêtes à 64 Mo sans modifier le contenu des images
application.config["SECRET_KEY"] = "face-compare-search-digest-secret-2026"  # Secret utilisé pour signer les données d’authentification Digest

digest_authentication = HTTPDigestAuth()  # Créer l’authentificateur HTTP Digest
ADMINISTRATOR_USERS = {"admin": "FaceAdmin@2026", "operator": "FaceOperator@2026"}  # Comptes administrateur intégrés ; à modifier selon les besoins

PROJECT_DIRECTORY = Path(__file__).resolve().parent  # Répertoire contenant ce script
...
FACE_DETECTION_MODEL = "cnn"  # Utiliser le détecteur de visages CNN
FACE_ENCODING_MODEL = "large"  # Utiliser le modèle large d’encodage facial
...
*{box-sizing:border-box}body{min-height:100vh;margin:0;color:var(--text);background:linear-gradient(135deg,#fff8f1,#ffefe0);font-family:Arial,Helvetica,sans-serif}a{color:inherit;text-decoration:none}button,input{font:inherit}
...
<body>
<header class="topbar"><div class="topbar-inner"><a class="brand" href="/compare">Reconnaissance faciale en ligne</a><nav class="navigation">{{ navigation_html|safe }}</nav></div></header>
...
<section class="hero"><h1>Comparaison de visages</h1><p>Téléversez deux images d’origine contenant chacune exactement un visage net. Le serveur utilise la détection CNN et le modèle large pour calculer la similarité.</p></section>
<section class="card">
<div class="upload-panel">
<label class="upload-box"><input id="firstFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="firstPlaceholder" class="upload-placeholder"><span class="upload-icon">①</span>Téléverser la première image d’origine</div><img id="firstPreview" class="preview-image" alt="Téléverser la première image d’origine"></label>
<label class="upload-box"><input id="secondFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="secondPlaceholder" class="upload-placeholder"><span class="upload-icon">②</span>Téléverser la deuxième image d’origine</div><img id="secondPreview" class="preview-image" alt="Téléverser la deuxième image d’origine"></label>
</div>
<div class="action-row"><button id="compareButton" class="primary-button" type="button">Comparer les visages</button><span id="compareStatus" class="status-text"></span></div>
<div id="compareResult" class="result-panel"><div class="similarity-label">Similarité faciale</div><div id="compareSimilarity" class="similarity-value">0.00%</div></div>
<p class="notice">Les fichiers téléversés sont conservés octet par octet, sans redimensionnement, réencodage ni compression. La correction de l’orientation et la conversion RGB sont effectuées uniquement en mémoire. La similarité affichée n’est pas une probabilité de vérification d’identité.</p>
</section>
"""

SEARCH_PAGE_CONTENT = """
<section class="hero"><h1>Recherche de visages</h1><p>Enregistrez des visages de référence, puis téléversez une image de recherche. Le serveur compare les enregistrements de la base de données séquentiellement tandis qu’AJAX affiche la progression et les résultats.</p></section>
<section class="card">
<h2>Gestion de la bibliothèque de visages</h2><p class="card-description">Chaque enregistrement conserve le nom d’une personne, l’image de référence d’origine et un encodage à 128 dimensions précalculé. Une même personne peut avoir plusieurs images de référence.</p>
<div class="authentication-panel"><button id="authenticateButton" class="secondary-button" type="button">Connexion administrateur</button></div>
<div class="form-row"><input id="personName" class="text-input" type="text" maxlength="80" placeholder="Saisissez le nom de la personne"><input id="libraryFaceImage" class="compact-upload" type="file" accept="image/jpeg,image/png,image/webp"></div>
<div class="action-row"><button id="addFaceButton" class="primary-button" type="button">Enregistrer le visage</button><span id="libraryStatus" class="status-text"></span></div>
<div class="section-divider"></div><div class="library-header"><h2>Visages enregistrés</h2><span id="libraryCount" class="library-count">0 images de référence</span></div>
<div id="libraryList" class="library-list"><div class="empty-state">Chargement de la bibliothèque de visages…</div></div>
</section>
<section class="card">
<h2>Recherche séquentielle de visages</h2><p class="card-description">La recherche compare les encodages faciaux enregistrés dans l’ordre et ne redétecte pas les visages dans les images de la bibliothèque.</p>
<div class="search-layout"><div><label id="searchPlaceholder" class="search-placeholder">Cliquez pour sélectionner une image de recherche d’origine<br>L’image doit contenir exactement un visage net<input id="searchFaceImage" type="file" accept="image/jpeg,image/png,image/webp"></label><img id="searchPreview" class="search-preview" alt="Aperçu de l’image de recherche"><div class="action-row"><button id="startSearchButton" class="primary-button" type="button">Lancer la recherche séquentielle</button></div></div>
<div><div id="searchMessage" class="status-text">En attente de démarrage</div><div id="progressArea" class="progress-area"><div class="progress-track"><div id="progressBar" class="progress-bar"></div></div><div class="progress-meta"><span id="progressText">0 / 0</span><span id="progressPercentage">0%</span></div><div id="currentPerson" class="current-person"></div></div><div id="searchResults" class="result-list"></div></div></div>
<p class="notice">Les résultats sont triés par ordre décroissant de similarité de comparaison faciale calculée par le modèle CNN.</p>
</section>
"""

ADMIN_AUTHENTICATION_PAGE = """
<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Authentification administrateur réussie</title>
<style>body{display:flex;min-height:100vh;margin:0;align-items:center;justify-content:center;color:#3d2c22;background:#fff4e8;font-family:Arial,Helvetica,sans-serif}.card{width:min(440px,calc(100% - 32px));padding:30px;border:1px solid #f1d5bf;border-radius:20px;background:#fffdf9;box-shadow:0 18px 45px rgba(152,92,45,.12);text-align:center}h1{margin-top:0;color:#c86625}p{color:#8a7060;line-height:1.8}button{padding:10px 20px;border:0;border-radius:12px;color:white;background:#e8873d;font:inherit;font-weight:700;cursor:pointer}</style></head>
<body><div class="card"><h1>Authentification administrateur réussie</h1><p>Administrateur actuel : {{ administrator_username }}. Vous pouvez revenir à la page de recherche pour enregistrer ou supprimer des visages.</p><button type="button" onclick="window.close()">Fermer la fenêtre</button></div></body></html>
"""
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}throw new Error(responseData.message||"La requête a échoué");}
async function compareSelectedFaces(){const firstFaceFile=firstFaceInput.files[0],secondFaceFile=secondFaceInput.files[0],statusElement=document.getElementById("compareStatus"),resultElement=document.getElementById("compareResult");if(!firstFaceFile||!secondFaceFile){statusElement.textContent="Sélectionnez d’abord deux images";statusElement.className="status-text error-text";return;}compareButton.disabled=true;resultElement.style.display="none";statusElement.textContent="Exécution de la détection CNN et de l’encodage facial avec le modèle large…";statusElement.className="status-text";const requestData=new FormData();requestData.append("first_face_image",firstFaceFile);requestData.append("second_face_image",secondFaceFile);try{const response=await fetch("/api/compare",{method:"POST",body:requestData});const responseData=await readJsonResponse(response);document.getElementById("compareSimilarity").textContent=Number(responseData.similarity).toFixed(2)+"%";resultElement.style.display="block";statusElement.textContent="Comparaison terminée";statusElement.className="status-text success-text";}catch(error){statusElement.textContent=error.message;statusElement.className="status-text error-text";}finally{compareButton.disabled=false;}}
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}if(response.status===401){throw new Error("L’authentification administrateur est absente ou invalide. Utilisez d’abord la connexion administrateur.");}throw new Error(responseData.message||"La requête a échoué");}
...
function renderFaceLibrary(faceRecords){const libraryList=document.getElementById("libraryList");document.getElementById("libraryCount").textContent=faceRecords.length+" images de référence";libraryList.replaceChildren();if(faceRecords.length===0){displayLibraryMessage("La bibliothèque de visages est vide.",false);return;}for(const faceRecord of faceRecords){libraryList.appendChild(createLibraryItem(faceRecord));}}
...
function openAdministratorAuthentication(){const authenticationWindow=window.open("/admin-auth","faceLibraryAuthentication","width=520,height=420");if(!authenticationWindow){setLibraryStatus("Le navigateur a bloqué la fenêtre d’authentification. Autorisez les fenêtres contextuelles, puis réessayez.",true);return;}setLibraryStatus("Saisissez le nom d’utilisateur et le mot de passe administrateur dans la fenêtre d’authentification",false);}
async function addFaceRecord(){const personName=personNameInput.value.trim(),faceImage=libraryFaceInput.files[0];if(!personName||!faceImage){setLibraryStatus("Saisissez le nom d’une personne et sélectionnez une image",true);return;}addFaceButton.disabled=true;setLibraryStatus("Exécution de la détection CNN et enregistrement de l’image d’origine et de l’encodage…",false);const requestData=new FormData();requestData.append("person_name",personName);requestData.append("face_image",faceImage);try{const response=await fetch("/api/library",{method:"POST",body:requestData,credentials:"same-origin"});await readJsonResponse(response);personNameInput.value="";libraryFaceInput.value="";setLibraryStatus("Enregistrement terminé",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}finally{addFaceButton.disabled=false;}}
async function deleteFaceRecord(faceRecordId,personName){if(!window.confirm("Supprimer cette image de référence pour « "+personName+" » ?")){return;}try{const response=await fetch("/api/library/"+faceRecordId,{method:"DELETE",credentials:"same-origin"});await readJsonResponse(response);setLibraryStatus("Suppression terminée",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}}
...
def get_administrator_password(username):
    return ADMINISTRATOR_USERS.get(username)  # Renvoyer le mot de passe utilisé par l’authentification Digest

@digest_authentication.error_handler
def handle_authentication_error(status_code=401):
    if request.path.startswith("/api/"):
        return jsonify({"message": "L’authentification Digest de l’administrateur est requise"}), status_code
    return Response("L’authentification Digest de l’administrateur est requise", status=status_code, content_type="text/plain; charset=utf-8")
...
def get_database_connection():
    database_connection = sqlite3.connect(DATABASE_PATH, timeout=30)  # Utiliser une connexion SQLite indépendante par requête ou par thread
...
def save_uploaded_image_unchanged(uploaded_image, destination_directory, filename_prefix):
    ...
    uploaded_image.save(generated_path)  # Conserver les octets téléversés sans modification, redimensionnement, réencodage ni compression
    try:
        with Image.open(generated_path) as source_image:
            source_image.verify()  # Vérifier la lisibilité sans modifier le fichier d’origine
    except (UnidentifiedImageError, OSError, ValueError) as image_error:
        generated_path.unlink(missing_ok=True)
        raise ValueError("L’image ne peut pas être lue ou le fichier est endommagé") from image_error
    return generated_path, generated_filename

def load_image_array_for_recognition(image_path):
    try:
        with Image.open(image_path) as source_image:
            oriented_image = ImageOps.exif_transpose(source_image)  # Corriger l’orientation EXIF uniquement en mémoire
            rgb_image = oriented_image.convert("RGB")  # Convertir en RGB uniquement en mémoire ; le fichier d’origine sur disque reste inchangé
...
def extract_single_face_encoding(image_path):
    ...
    face_locations = face_recognition.face_locations(image_array, number_of_times_to_upsample=FACE_LOCATION_UPSAMPLE_COUNT, model=FACE_DETECTION_MODEL)  # Détecter exactement un visage avec le CNN
    if len(face_locations) == 0:
        raise ValueError("Aucun visage net n’a été détecté dans l’image")
    if len(face_locations) > 1:
        raise ValueError("Plusieurs visages ont été détectés ; utilisez une image ne contenant qu’un seul visage")
    face_encodings = face_recognition.face_encodings(image_array, known_face_locations=face_locations, num_jitters=FACE_ENCODING_JITTER_COUNT, model=FACE_ENCODING_MODEL)  # Extraire l’encodage à 128 dimensions avec le modèle large
    if not face_encodings:
        raise ValueError("L’encodage facial a échoué ; utilisez une image plus nette")
...
# =============================================================================
# Tâches de recherche en arrière-plan
# =============================================================================
...
def run_sequential_face_search(search_job_id, search_face_encoding):
    ...
    try:
        ...
        for face_record in face_records:  # Comparer strictement dans l’ordre de la base de données
            person_name = str(face_record["person_name"])
        ...
        update_search_job(search_job_id, {"status": "error", "current_person": "", "message": "Échec de la recherche de visages : " + str(search_error)})

# =============================================================================
# Routes des pages et de l’API
# =============================================================================
...
def render_application_page(page_title, active_page, page_content, page_script):
    ...
    navigation_html = '<a class="' + compare_class + '" href="/compare">Comparaison</a><a class="' + search_class + '" href="/search">Recherche de visages</a>'
...
def compare_page():
    return render_application_page("Comparaison de visages", "compare", COMPARE_PAGE_CONTENT, COMPARE_PAGE_SCRIPT)

@application.route("/search")
def search_page():
    return render_application_page("Recherche de visages", "search", SEARCH_PAGE_CONTENT, SEARCH_PAGE_SCRIPT)
...
def compare_faces_api():
    ...
    try:
        ...
        return jsonify({"message": "Échec de la comparaison de visages : " + str(unexpected_error)}), 500
    finally:
...
def add_face_library_api():
    ...
    try:
        ...
        if not person_name:
            raise ValueError("Saisissez le nom de la personne")
        if len(person_name) > 80:
            raise ValueError("Le nom de la personne ne peut pas dépasser 80 caractères")
        ...
        remove_file_safely(stored_image_path)
        return jsonify({"message": "Échec de l’enregistrement du visage : " + str(unexpected_error)}), 500
...
def delete_face_library_api(face_record_id):
    ...
    if image_filename is None:
        return jsonify({"message": "L’enregistrement facial demandé n’existe pas"}), 404
...
def start_face_search_api():
    ...
    try:
        ...
        if face_library_count == 0:
            raise ValueError("La bibliothèque de visages est vide ; enregistrez d’abord un visage")
        ...
        search_thread = Thread(target=run_sequential_face_search, args=(search_job_id, search_face_encoding.copy()), daemon=True, name="face-search-" + search_job_id[:8])  # Maintenir la réactivité de l’interrogation AJAX pendant la recherche
        search_thread.start()
        return jsonify({"search_job_id": search_job_id}), 202
    except ValueError as validation_error:
        return jsonify({"message": str(validation_error)}), 400
    except Exception as unexpected_error:
        return jsonify({"message": "La recherche n’a pas pu être lancée : " + str(unexpected_error)}), 500
    finally:
...
def search_status_api(search_job_id):
    ...
    if search_job is None:
        return jsonify({"message": "La tâche de recherche n’existe pas ou a expiré"}), 404
...
def request_too_large(_error):
    if request.path.startswith("/api/"):
        return jsonify({"message": "La requête téléversée est trop volumineuse ; la taille maximale est de 64 Mo"}), 413
    return "La requête téléversée est trop volumineuse ; la taille maximale est de 64 Mo", 413
...
if __name__ == "__main__":
    application.run(host="0.0.0.0", port=8080, debug=False, threaded=True)  # Serveur de développement ; utiliser WSGI et HTTPS en production
