...
application.config["MAX_CONTENT_LENGTH"] = 64 * 1024 * 1024  # Limit requests to 64 MB without changing image content
application.config["SECRET_KEY"] = "face-compare-search-digest-secret-2026"  # Secret used to sign Digest authentication data

digest_authentication = HTTPDigestAuth()  # Create the HTTP Digest authenticator
ADMINISTRATOR_USERS = {"admin": "FaceAdmin@2026", "operator": "FaceOperator@2026"}  # Built-in administrator accounts; edit as needed

PROJECT_DIRECTORY = Path(__file__).resolve().parent  # Directory containing this script
...
FACE_DETECTION_MODEL = "cnn"  # Use the CNN face detector
FACE_ENCODING_MODEL = "large"  # Use the large face encoding model
...
<body>
<header class="topbar"><div class="topbar-inner"><a class="brand" href="/compare">Face Reco Online</a><nav class="navigation">{{ navigation_html|safe }}</nav></div></header>
...
<section class="hero"><h1>Face Comparison</h1><p>Upload two original images containing exactly one clear face. The server uses CNN detection and the large model to calculate similarity.</p></section>
<section class="card">
<div class="upload-panel">
<label class="upload-box"><input id="firstFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="firstPlaceholder" class="upload-placeholder"><span class="upload-icon">①</span>Upload the first original image</div><img id="firstPreview" class="preview-image" alt="Upload the first original image"></label>
<label class="upload-box"><input id="secondFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="secondPlaceholder" class="upload-placeholder"><span class="upload-icon">②</span>Upload the second original image</div><img id="secondPreview" class="preview-image" alt="Upload the second original image"></label>
</div>
<div class="action-row"><button id="compareButton" class="primary-button" type="button">Compare Faces</button><span id="compareStatus" class="status-text"></span></div>
<div id="compareResult" class="result-panel"><div class="similarity-label">Face Similarity</div><div id="compareSimilarity" class="similarity-value">0.00%</div></div>
<p class="notice">Uploaded files are stored byte-for-byte without resizing, re-encoding, or compression. Orientation correction and RGB conversion happen only in memory. The displayed similarity is not an identity-verification probability.</p>
</section>
"""
...
<div id="libraryList" class="library-list"><div class="empty-state">Loading the face library…</div></div>
</section>
<section class="card">
...
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Administrator Authentication Successful</title>
...
<body><div class="card"><h1>Administrator Authentication Successful</h1><p>Current administrator: {{ administrator_username }}. You may return to the search page to enroll or delete faces.</p><button type="button" onclick="window.close()">Close Window</button></div></body></html>
"""
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}throw new Error(responseData.message||"The request failed");}
async function compareSelectedFaces(){const firstFaceFile=firstFaceInput.files[0],secondFaceFile=secondFaceInput.files[0],statusElement=document.getElementById("compareStatus"),resultElement=document.getElementById("compareResult");if(!firstFaceFile||!secondFaceFile){statusElement.textContent="Select two images first";statusElement.className="status-text error-text";return;}compareButton.disabled=true;resultElement.style.display="none";statusElement.textContent="Running CNN detection and large face encoding…";statusElement.className="status-text";const requestData=new FormData();requestData.append("first_face_image",firstFaceFile);requestData.append("second_face_image",secondFaceFile);try{const response=await fetch("/api/compare",{method:"POST",body:requestData});const responseData=await readJsonResponse(response);document.getElementById("compareSimilarity").textContent=Number(responseData.similarity).toFixed(2)+"%";resultElement.style.display="block";statusElement.textContent="Comparison completed";statusElement.className="status-text success-text";}catch(error){statusElement.textContent=error.message;statusElement.className="status-text error-text";}finally{compareButton.disabled=false;}}
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}if(response.status===401){throw new Error("Administrator authentication is missing or invalid. Use Administrator Login first.");}throw new Error(responseData.message||"The request failed");}
...
function renderFaceLibrary(faceRecords){const libraryList=document.getElementById("libraryList");document.getElementById("libraryCount").textContent=faceRecords.length+" reference images";libraryList.replaceChildren();if(faceRecords.length===0){displayLibraryMessage("The face library is empty.",false);return;}for(const faceRecord of faceRecords){libraryList.appendChild(createLibraryItem(faceRecord));}}
...
function openAdministratorAuthentication(){const authenticationWindow=window.open("/admin-auth","faceLibraryAuthentication","width=520,height=420");if(!authenticationWindow){setLibraryStatus("The browser blocked the authentication window. Allow pop-ups and try again.",true);return;}setLibraryStatus("Enter the administrator username and password in the authentication window",false);}
async function addFaceRecord(){const personName=personNameInput.value.trim(),faceImage=libraryFaceInput.files[0];if(!personName||!faceImage){setLibraryStatus("Enter a person name and select an image",true);return;}addFaceButton.disabled=true;setLibraryStatus("Running CNN detection and storing the original image and encoding…",false);const requestData=new FormData();requestData.append("person_name",personName);requestData.append("face_image",faceImage);try{const response=await fetch("/api/library",{method:"POST",body:requestData,credentials:"same-origin"});await readJsonResponse(response);personNameInput.value="";libraryFaceInput.value="";setLibraryStatus("Enrollment completed",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}finally{addFaceButton.disabled=false;}}
async function deleteFaceRecord(faceRecordId,personName){if(!window.confirm("Delete this reference image for “"+personName+"”?")){return;}try{const response=await fetch("/api/library/"+faceRecordId,{method:"DELETE",credentials:"same-origin"});await readJsonResponse(response);setLibraryStatus("Deletion completed",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}}
...
def get_administrator_password(username):
    return ADMINISTRATOR_USERS.get(username)  # Return the password used by Digest authentication

@digest_authentication.error_handler
def handle_authentication_error(status_code=401):
    if request.path.startswith("/api/"):
        return jsonify({"message": "Administrator Digest authentication is required"}), status_code
    return Response("Administrator Digest authentication is required", status=status_code, content_type="text/plain; charset=utf-8")
...
def get_database_connection():
    database_connection = sqlite3.connect(DATABASE_PATH, timeout=30)  # Use an independent SQLite connection per request or thread
...
def save_uploaded_image_unchanged(uploaded_image, destination_directory, filename_prefix):
    ...
    uploaded_image.save(generated_path)  # Store uploaded bytes unchanged; no resize, re-encoding, or compression
    try:
        with Image.open(generated_path) as source_image:
            source_image.verify()  # Verify readability without modifying the original file
    except (UnidentifiedImageError, OSError, ValueError) as image_error:
        generated_path.unlink(missing_ok=True)
        raise ValueError("The image cannot be read or the file is damaged") from image_error
    return generated_path, generated_filename

def load_image_array_for_recognition(image_path):
    try:
        with Image.open(image_path) as source_image:
            oriented_image = ImageOps.exif_transpose(source_image)  # Correct EXIF orientation only in memory
            rgb_image = oriented_image.convert("RGB")  # Convert to RGB only in memory; the disk original remains unchanged
...
def extract_single_face_encoding(image_path):
    ...
    face_locations = face_recognition.face_locations(image_array, number_of_times_to_upsample=FACE_LOCATION_UPSAMPLE_COUNT, model=FACE_DETECTION_MODEL)  # Detect exactly one face with CNN
    if len(face_locations) == 0:
        raise ValueError("No clear face was detected in the image")
    if len(face_locations) > 1:
        raise ValueError("Multiple faces were detected; use an image containing one face only")
    face_encodings = face_recognition.face_encodings(image_array, known_face_locations=face_locations, num_jitters=FACE_ENCODING_JITTER_COUNT, model=FACE_ENCODING_MODEL)  # Extract the 128-dimensional encoding with the large model
    if not face_encodings:
        raise ValueError("Face encoding failed; use a clearer image")
...
def run_sequential_face_search(search_job_id, search_face_encoding):
    ...
    try:
        ...
        for face_record in face_records:  # Compare strictly in database order
            person_name = str(face_record["person_name"])
        ...
        update_search_job(search_job_id, {"status": "error", "current_person": "", "message": "Face search failed: " + str(search_error)})
...
def render_application_page(page_title, active_page, page_content, page_script):
    ...
    navigation_html = '<a class="' + compare_class + '" href="/compare">Face Compare</a><a class="' + search_class + '" href="/search">Face Search</a>'
...
def compare_page():
    return render_application_page("Face Comparison", "compare", COMPARE_PAGE_CONTENT, COMPARE_PAGE_SCRIPT)

@application.route("/search")
def search_page():
    return render_application_page("Face Search", "search", SEARCH_PAGE_CONTENT, SEARCH_PAGE_SCRIPT)
...
def compare_faces_api():
    ...
    try:
        ...
        return jsonify({"message": "Face comparison failed: " + str(unexpected_error)}), 500
    finally:
...
def add_face_library_api():
    ...
    try:
        ...
        if not person_name:
            raise ValueError("Enter the person name")
        if len(person_name) > 80:
            raise ValueError("The person name cannot exceed 80 characters")
        ...
        remove_file_safely(stored_image_path)
        return jsonify({"message": "Face enrollment failed: " + str(unexpected_error)}), 500
...
def delete_face_library_api(face_record_id):
    ...
    if image_filename is None:
        return jsonify({"message": "The requested face record does not exist"}), 404
...
def start_face_search_api():
    ...
    try:
        ...
        if face_library_count == 0:
            raise ValueError("The face library is empty; enroll a face first")
        ...
        search_thread = Thread(target=run_sequential_face_search, args=(search_job_id, search_face_encoding.copy()), daemon=True, name="face-search-" + search_job_id[:8])  # Keep AJAX polling responsive while the search runs
        search_thread.start()
        return jsonify({"search_job_id": search_job_id}), 202
    except ValueError as validation_error:
        return jsonify({"message": str(validation_error)}), 400
    except Exception as unexpected_error:
        return jsonify({"message": "The search could not be started: " + str(unexpected_error)}), 500
    finally:
...
def search_status_api(search_job_id):
    ...
    if search_job is None:
        return jsonify({"message": "The search job does not exist or has expired"}), 404
...
def request_too_large(_error):
    if request.path.startswith("/api/"):
        return jsonify({"message": "The uploaded request is too large; the maximum request size is 64 MB"}), 413
    return "The uploaded request is too large; the maximum request size is 64 MB", 413
...
if __name__ == "__main__":
    application.run(host="0.0.0.0", port=8080, debug=False, threaded=True)  # Development server; use WSGI and HTTPS in production
