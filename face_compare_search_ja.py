"""
この単一ファイルの Flask アプリケーションは、ブラウザー上で利用できる顔比較機能と顔ライブラリー検索機能を 1 つのプログラムとして提供します。
アップロードされた 2 枚の顔画像を比較し、ローカルの参照ライブラリーを管理するとともに、検索用の顔を保存済みの 128 次元エンコーディングと照合します。
検索レコードはバックグラウンドスレッドによってデータベース ID の昇順で順次処理され、AJAX ポーリングにより進行状況と順位付けされた結果がリアルタイムで表示されます。
顔検出には face_recognition を介した dlib の CNN モデルを使用し、顔エンコーディングには 68 点の顔ランドマークを用いる large モデルを使用します。アップロードする各画像には、検出可能な顔が正確に 1 つだけ含まれている必要があります。画像ファイルは、サイズ変更、切り抜き、形式変換、再エンコード、圧縮を行わずに保存されます。EXIF 方向補正と RGB 変換は、認識処理中にメモリー上でのみ実行されます。SQLite には、人物名、参照画像のファイル名、JSON 形式の顔ベクトル、作成日時が保存されます。顔ライブラリーへの登録またはレコードの削除を行う場合にのみ、HTTP Digest 管理者認証が必要です。
"""
...
application = Flask(__name__)  # Flask アプリケーションを作成
application.config["MAX_CONTENT_LENGTH"] = 64 * 1024 * 1024  # 画像内容を変更せず、リクエストサイズを 64 MB に制限
application.config["SECRET_KEY"] = "face-compare-search-digest-secret-2026"  # Digest 認証データの署名に使用する秘密鍵

digest_authentication = HTTPDigestAuth()  # HTTP Digest 認証オブジェクトを作成
ADMINISTRATOR_USERS = {"admin": "FaceAdmin@2026", "operator": "FaceOperator@2026"}  # 組み込みの管理者アカウント。必要に応じて編集してください

PROJECT_DIRECTORY = Path(__file__).resolve().parent  # このスクリプトが格納されているディレクトリ
...
FACE_DETECTION_MODEL = "cnn"  # CNN 顔検出モデルを使用
FACE_ENCODING_MODEL = "large"  # large 顔エンコーディングモデルを使用
...
*{box-sizing:border-box}body{min-height:100vh;margin:0;color:var(--text);background:linear-gradient(135deg,#fff8f1,#ffefe0);font-family:"Yu Gothic","Hiragino Kaku Gothic ProN","Meiryo",Arial,sans-serif}a{color:inherit;text-decoration:none}button,input{font:inherit}
...
<body>
<header class="topbar"><div class="topbar-inner"><a class="brand" href="/compare">オンライン顔認識</a><nav class="navigation">{{ navigation_html|safe }}</nav></div></header>
...
<section class="hero"><h1>顔比較</h1><p>鮮明な顔が正確に 1 つだけ写っている元画像を 2 枚アップロードしてください。サーバーは CNN 検出と large モデルを使用して類似度を計算します。</p></section>
<section class="card">
<div class="upload-panel">
<label class="upload-box"><input id="firstFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="firstPlaceholder" class="upload-placeholder"><span class="upload-icon">①</span>1 枚目の元画像をアップロード</div><img id="firstPreview" class="preview-image" alt="1 枚目の元画像をアップロード"></label>
<label class="upload-box"><input id="secondFaceImage" type="file" accept="image/jpeg,image/png,image/webp"><div id="secondPlaceholder" class="upload-placeholder"><span class="upload-icon">②</span>2 枚目の元画像をアップロード</div><img id="secondPreview" class="preview-image" alt="2 枚目の元画像をアップロード"></label>
</div>
<div class="action-row"><button id="compareButton" class="primary-button" type="button">顔を比較</button><span id="compareStatus" class="status-text"></span></div>
<div id="compareResult" class="result-panel"><div class="similarity-label">顔の類似度</div><div id="compareSimilarity" class="similarity-value">0.00%</div></div>
<p class="notice">アップロードされたファイルは、サイズ変更、再エンコード、圧縮を行わず、バイト単位でそのまま保存されます。向きの補正と RGB 変換はメモリー上でのみ実行されます。表示される類似度は、本人確認の確率ではありません。</p>
</section>
"""

SEARCH_PAGE_CONTENT = """
<section class="hero"><h1>顔検索</h1><p>参照用の顔を登録してから、検索用画像をアップロードしてください。サーバーはデータベースのレコードを順番に比較し、AJAX で進行状況と結果を表示します。</p></section>
<section class="card">
<h2>顔ライブラリー管理</h2><p class="card-description">各レコードには、人物名、元の参照画像、事前計算済みの 128 次元エンコーディングが保存されます。同一人物に複数の参照画像を登録できます。</p>
<div class="authentication-panel"><button id="authenticateButton" class="secondary-button" type="button">管理者ログイン</button></div>
<div class="form-row"><input id="personName" class="text-input" type="text" maxlength="80" placeholder="人物名を入力してください"><input id="libraryFaceImage" class="compact-upload" type="file" accept="image/jpeg,image/png,image/webp"></div>
<div class="action-row"><button id="addFaceButton" class="primary-button" type="button">顔を登録</button><span id="libraryStatus" class="status-text"></span></div>
<div class="section-divider"></div><div class="library-header"><h2>登録済みの顔</h2><span id="libraryCount" class="library-count">参照画像 0 件</span></div>
<div id="libraryList" class="library-list"><div class="empty-state">顔ライブラリーを読み込んでいます…</div></div>
</section>
<section class="card">
<h2>順次顔検索</h2><p class="card-description">検索では、保存済みの顔エンコーディングを順番に比較し、ライブラリー画像から顔を再検出しません。</p>
<div class="search-layout"><div><label id="searchPlaceholder" class="search-placeholder">クリックして元の検索用画像を選択<br>画像には鮮明な顔が正確に 1 つだけ含まれている必要があります<input id="searchFaceImage" type="file" accept="image/jpeg,image/png,image/webp"></label><img id="searchPreview" class="search-preview" alt="検索用画像のプレビュー"><div class="action-row"><button id="startSearchButton" class="primary-button" type="button">順次検索を開始</button></div></div>
<div><div id="searchMessage" class="status-text">開始待ち</div><div id="progressArea" class="progress-area"><div class="progress-track"><div id="progressBar" class="progress-bar"></div></div><div class="progress-meta"><span id="progressText">0 / 0</span><span id="progressPercentage">0%</span></div><div id="currentPerson" class="current-person"></div></div><div id="searchResults" class="result-list"></div></div></div>
<p class="notice">結果は、CNN モデルで算出された顔比較の類似度が高い順に並べられます。</p>
</section>
"""

ADMIN_AUTHENTICATION_PAGE = """
<!doctype html><html lang="ja"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>管理者認証に成功しました</title>
<style>body{display:flex;min-height:100vh;margin:0;align-items:center;justify-content:center;color:#3d2c22;background:#fff4e8;font-family:"Yu Gothic","Hiragino Kaku Gothic ProN","Meiryo",Arial,sans-serif}.card{width:min(440px,calc(100% - 32px));padding:30px;border:1px solid #f1d5bf;border-radius:20px;background:#fffdf9;box-shadow:0 18px 45px rgba(152,92,45,.12);text-align:center}h1{margin-top:0;color:#c86625}p{color:#8a7060;line-height:1.8}button{padding:10px 20px;border:0;border-radius:12px;color:white;background:#e8873d;font:inherit;font-weight:700;cursor:pointer}</style></head>
<body><div class="card"><h1>管理者認証に成功しました</h1><p>現在の管理者: {{ administrator_username }}。検索ページに戻って、顔の登録または削除を行えます。</p><button type="button" onclick="window.close()">ウィンドウを閉じる</button></div></body></html>
"""
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}throw new Error(responseData.message||"リクエストに失敗しました");}
async function compareSelectedFaces(){const firstFaceFile=firstFaceInput.files[0],secondFaceFile=secondFaceInput.files[0],statusElement=document.getElementById("compareStatus"),resultElement=document.getElementById("compareResult");if(!firstFaceFile||!secondFaceFile){statusElement.textContent="先に 2 枚の画像を選択してください";statusElement.className="status-text error-text";return;}compareButton.disabled=true;resultElement.style.display="none";statusElement.textContent="CNN 検出と large 顔エンコーディングを実行しています…";statusElement.className="status-text";const requestData=new FormData();requestData.append("first_face_image",firstFaceFile);requestData.append("second_face_image",secondFaceFile);try{const response=await fetch("/api/compare",{method:"POST",body:requestData});const responseData=await readJsonResponse(response);document.getElementById("compareSimilarity").textContent=Number(responseData.similarity).toFixed(2)+"%";resultElement.style.display="block";statusElement.textContent="比較が完了しました";statusElement.className="status-text success-text";}catch(error){statusElement.textContent=error.message;statusElement.className="status-text error-text";}finally{compareButton.disabled=false;}}
...
async function readJsonResponse(response){let responseData={};try{responseData=await response.json();}catch(error){responseData={};}if(response.ok){return responseData;}if(response.status===401){throw new Error("管理者認証がないか、無効です。先に管理者ログインを実行してください。");}throw new Error(responseData.message||"リクエストに失敗しました");}
...
function openAdministratorAuthentication(){const authenticationWindow=window.open("/admin-auth","faceLibraryAuthentication","width=520,height=420");if(!authenticationWindow){setLibraryStatus("ブラウザーによって認証ウィンドウがブロックされました。ポップアップを許可して、もう一度お試しください。",true);return;}setLibraryStatus("認証ウィンドウに管理者のユーザー名とパスワードを入力してください",false);}
async function addFaceRecord(){const personName=personNameInput.value.trim(),faceImage=libraryFaceInput.files[0];if(!personName||!faceImage){setLibraryStatus("人物名を入力し、画像を選択してください",true);return;}addFaceButton.disabled=true;setLibraryStatus("CNN 検出を実行し、元画像とエンコーディングを保存しています…",false);const requestData=new FormData();requestData.append("person_name",personName);requestData.append("face_image",faceImage);try{const response=await fetch("/api/library",{method:"POST",body:requestData,credentials:"same-origin"});await readJsonResponse(response);personNameInput.value="";libraryFaceInput.value="";setLibraryStatus("登録が完了しました",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}finally{addFaceButton.disabled=false;}}
async function deleteFaceRecord(faceRecordId,personName){if(!window.confirm("「"+personName+"」のこの参照画像を削除しますか？")){return;}try{const response=await fetch("/api/library/"+faceRecordId,{method:"DELETE",credentials:"same-origin"});await readJsonResponse(response);setLibraryStatus("削除が完了しました",false);await loadFaceLibrary();}catch(error){setLibraryStatus(error.message,true);}}
...
def get_administrator_password(username):
    return ADMINISTRATOR_USERS.get(username)  # Digest 認証で使用するパスワードを返す

@digest_authentication.error_handler
def handle_authentication_error(status_code=401):
    if request.path.startswith("/api/"):
        return jsonify({"message": "管理者の Digest 認証が必要です"}), status_code
    return Response("管理者の Digest 認証が必要です", status=status_code, content_type="text/plain; charset=utf-8")
...
def get_database_connection():
    database_connection = sqlite3.connect(DATABASE_PATH, timeout=30)  # リクエストまたはスレッドごとに独立した SQLite 接続を使用
...
def validate_uploaded_image(uploaded_image):
    ...
    if image_extension not in ALLOWED_IMAGE_EXTENSIONS:
        raise ValueError("JPG、JPEG、PNG、WebP 画像のみ対応しています")
...
def save_uploaded_image_unchanged(uploaded_image, destination_directory, filename_prefix):
    ...
    uploaded_image.save(generated_path)  # アップロードされたバイト列を変更せずに保存。サイズ変更、再エンコード、圧縮は行わない
    try:
        with Image.open(generated_path) as source_image:
            source_image.verify()  # 元ファイルを変更せずに読み取り可能か確認
    except (UnidentifiedImageError, OSError, ValueError) as image_error:
        generated_path.unlink(missing_ok=True)
        raise ValueError("画像を読み取れないか、ファイルが破損しています") from image_error
    return generated_path, generated_filename

def load_image_array_for_recognition(image_path):
    try:
        with Image.open(image_path) as source_image:
            oriented_image = ImageOps.exif_transpose(source_image)  # EXIF の向きをメモリー上でのみ補正
            rgb_image = oriented_image.convert("RGB")  # RGB 変換はメモリー上でのみ実行し、ディスク上の元ファイルは変更しない
...
def extract_single_face_encoding(image_path):
    ...
    face_locations = face_recognition.face_locations(image_array, number_of_times_to_upsample=FACE_LOCATION_UPSAMPLE_COUNT, model=FACE_DETECTION_MODEL)  # CNN で顔を正確に 1 つ検出
    if len(face_locations) == 0:
        raise ValueError("画像から鮮明な顔を検出できませんでした")
    if len(face_locations) > 1:
        raise ValueError("複数の顔が検出されました。顔が 1 つだけ写っている画像を使用してください")
    face_encodings = face_recognition.face_encodings(image_array, known_face_locations=face_locations, num_jitters=FACE_ENCODING_JITTER_COUNT, model=FACE_ENCODING_MODEL)  # large モデルで 128 次元エンコーディングを抽出
    if not face_encodings:
        raise ValueError("顔のエンコーディングに失敗しました。より鮮明な画像を使用してください")
...
def run_sequential_face_search(search_job_id, search_face_encoding):
    ...
    try:
        ...
        for face_record in face_records:  # データベースの順序どおりに厳密に比較
            person_name = str(face_record["person_name"])
        ...
        update_search_job(search_job_id, {"status": "error", "current_person": "", "message": "顔検索に失敗しました: " + str(search_error)})
...
def render_application_page(page_title, active_page, page_content, page_script):
    ...
    navigation_html = '<a class="' + compare_class + '" href="/compare">顔比較</a><a class="' + search_class + '" href="/search">顔検索</a>'
...
def compare_page():
    return render_application_page("顔比較", "compare", COMPARE_PAGE_CONTENT, COMPARE_PAGE_SCRIPT)

@application.route("/search")
def search_page():
    return render_application_page("顔検索", "search", SEARCH_PAGE_CONTENT, SEARCH_PAGE_SCRIPT)
...
def compare_faces_api():
    ...
    try:
        ...
        return jsonify({"message": "顔比較に失敗しました: " + str(unexpected_error)}), 500
    finally:
...
def add_face_library_api():
    ...
    try:
        ...
        if not person_name:
            raise ValueError("人物名を入力してください")
        if len(person_name) > 80:
            raise ValueError("人物名は 80 文字以内で入力してください")
        ...
        remove_file_safely(stored_image_path)
        return jsonify({"message": "顔の登録に失敗しました: " + str(unexpected_error)}), 500
...
def delete_face_library_api(face_record_id):
    ...
    if image_filename is None:
        return jsonify({"message": "指定された顔レコードは存在しません"}), 404
...
def start_face_search_api():
    ...
    try:
        ...
        if face_library_count == 0:
            raise ValueError("顔ライブラリーは空です。先に顔を登録してください")
        ...
        search_thread = Thread(target=run_sequential_face_search, args=(search_job_id, search_face_encoding.copy()), daemon=True, name="face-search-" + search_job_id[:8])  # 検索中も AJAX ポーリングの応答性を維持
        search_thread.start()
        return jsonify({"search_job_id": search_job_id}), 202
    except ValueError as validation_error:
        return jsonify({"message": str(validation_error)}), 400
    except Exception as unexpected_error:
        return jsonify({"message": "検索を開始できませんでした: " + str(unexpected_error)}), 500
    finally:
...
def search_status_api(search_job_id):
    ...
    if search_job is None:
        return jsonify({"message": "検索ジョブが存在しないか、有効期限が切れています"}), 404
...
def request_too_large(_error):
    if request.path.startswith("/api/"):
        return jsonify({"message": "アップロードされたリクエストが大きすぎます。最大リクエストサイズは 64 MB です"}), 413
    return "アップロードされたリクエストが大きすぎます。最大リクエストサイズは 64 MB です", 413
...
if __name__ == "__main__":
    application.run(host="0.0.0.0", port=8080, debug=False, threaded=True)  # 開発用サーバー。本番環境では WSGI と HTTPS を使用してください
