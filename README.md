...
Run only one edition at a time. Both files listen on port `8080` and use the same `face_data` directory.
...
- Preserve uploaded image files without resizing, cropping, format conversion, re-encoding, or additional compression.
...
Change these passwords and `application.config["SECRET_KEY"]` before making the service publicly accessible.
