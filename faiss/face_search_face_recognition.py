...
for image_path in database_image_paths:
    ...
    largest_face_location = max(
        face_locations,
        key=lambda location: (location[2] - location[0]) * (location[1] - location[3]),
    )
    face_encodings = face_recognition.face_encodings(
        image,
        known_face_locations=[largest_face_location],
        num_jitters=1,
        model="large",
    )

    if not face_encodings:
        print(f"Skipped {image_path.name}: face encoding failed")
        continue

    face_names.append(image_path.stem)
    face_vectors.append(face_encodings[0].astype("float32"))
    print(f"Loaded: {image_path.name}")

if not face_vectors:
    raise RuntimeError("No valid face encodings were loaded from the faces directory.")
...
largest_query_location = max(
    query_face_locations,
    key=lambda location: (location[2] - location[0]) * (location[1] - location[3]),
)
query_encodings = face_recognition.face_encodings(
    query_image,
    known_face_locations=[largest_query_location],
    num_jitters=1,
    model="large",
)

if not query_encodings:
    raise RuntimeError(f"Face encoding failed for query image: {query_image_path}")

query_vector = np.ascontiguousarray([query_encodings[0]], dtype="float32")
...
print("\n" + "=" * 70)
print("FACE_RECOGNITION VECTOR SEARCH RESULTS")
print("=" * 70)
